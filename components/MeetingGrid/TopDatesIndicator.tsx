...
interface TopDatesIndicatorProps {
  topDates: TopDate[];
  datePositions: { [dateSlotKey: string]: number };
  scrollTop: number;
  clientHeight: number;
...
export const TopDatesIndicator = memo(function TopDatesIndicator({
  ...
  return (
    <div className="fixed right-4 top-1/2 transform -translate-y-1/2 flex flex-col gap-2 z-50">
      {topDates.map((item) => {
        const position = datePositions[item.dateSlotKey];
        ...
        return (
          <button
            key={item.dateSlotKey}
            onClick={() => onDateClick(item.dateSlotKey)}
